...
const HospitalListScreen: React.FC<HospitalListScreenProps> = ({ onSelect }) => {
  ...
  const handleHospitalSelect = (hospital: any) => {
    ...
    // Check if this is a regular appointment flow that requires login
    if (state.situation !== 'emergency' && !authState.user) {
      // Regular appointment requires patient login
...
<<<<<<< HEAD
        ...
        <AuthModal
          ...
          onUserTypeSelect={(userType: UserType, action: 'login' | 'signup') => {
            // For hospital selection, we only allow patient login
            if (userType === 'patient') {
